...
bool Bsp::validate()
{
	...
	for (int i = 0; i < leafCount; i++)
	{
		...
		for (int n = 0; n < 3; n++)
		{
			...
			{
				logf("backwards mins / maxs in leaf %d Mins: (%f, %f, %f) Maxs: (%f %f %f)", i, leaves[i].nMins[0], leaves[i].nMins[1], leaves[i].nMins[2],
